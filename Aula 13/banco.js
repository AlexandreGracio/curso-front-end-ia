...
campTitular.addEventListener('input', () => {
  titular = document.querySelector('#titular')
  titular.textContent = campTitular.value
...
function depositar(valor) {
  if (!contaAtiva) {
    ...
    saldo += valor;
    saldostring = saldo.toLocaleString('pt-BR', {  style: 'currency', currency: 'BRL'})
    valorstring = valor.toLocaleString('pt-BR', {  style: 'currency', currency: 'BRL'})
    //atualizarExtrato(`Depósito: R$ ${valor.toFixed(2)} | Saldo: R$ ${saldo.toFixed(2)}`);
    atualizarExtrato(`Depósito: R$ ${valorstring} | Saldo: R$ ${saldostring}`);
    atualizarSaldo()
    verResumo()
...
function sacar(valor) {
  if (!contaAtiva) {
    ...
    atualizarExtrato(`Saque: R$ ${valorstring} | Saldo: R$ ${saldostring}`);
    atualizarSaldo()
    verResumo()
...
function atualizarSaldo() {
  saldostring = saldo.toLocaleString('pt-BR', {  style: 'currency', currency: 'BRL'})
  elSaldo.textContent = `${saldostring}`
  if (saldo <= 1000) {
    elSaldo.style.color = '#991b1b';
  } else {
    if (saldo <= 5000) {
      elSaldo.style.color = '#99861b'
    } else {
      elSaldo.style.color = '#1b9930';
    }
  }
}
