...
function criarTabuleiro(num_linhas, num_colunas) {
...
function posicionarNavios(num_navios) {
  
  //const num_navios = 3

  for (let i = 0; i < num_navios; i++) {

    let lin_aleatoria = Math.floor(Math.random() * num_linhas) 
    let col_aleatoria = Math.floor(Math.random() * num_colunas)

    while (tabuleiro[lin_aleatoria][col_aleatoria] === 1) {
      lin_aleatoria = Math.floor(Math.random() * num_linhas) 
      col_aleatoria = Math.floor(Math.random() * num_colunas)
    }
    tabuleiro[lin_aleatoria][col_aleatoria] = 1;
    //console.log(tabuleiro[lin_aleatoria][col_aleatoria])
  }
...
function atirar(lin, col) {
  ...
  const conteudo_do_tiro = tabuleiro[lin][col]  

  // Se a célula já foi atingida (valor 2 ou 3):
  if (conteudo_do_tiro === status.Acerto || conteudo_do_tiro === status.Erro) {
    // Exibir "Posição já atingida!" no console
  ...
  // Se for navio (valor 1):
  if (conteudo_do_tiro === status.Navio) { 
    //Alterar o valor da celula para 2
    tabuleiro[lin][col] = status.Acerto
  ...
  // Se for água (valor 0):
    if (conteudo_do_tiro === status.Agua) { 
    // Alterar o valor da célula para 3
    tabuleiro[lin][col] = status.Erro
...
function iniciarJogo() {
  ...
  criarTabuleiro(num_linhas, num_colunas);

  naviosRestantes = num_navios;
  tentativas = 0;
  jogoAtivo = true;

  posicionarNavios(num_navios);
